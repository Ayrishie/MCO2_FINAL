...
/**
 * The TestRegularMachineView class represents the graphical user interface (GUI)
 * for testing a regular vending machine. It allows users to select items, make payments,
...
public class TestRegularMachineView {
    ...
    /**
     * Constructs a TestRegularMachineView instance with the specified RegularVendingMachine.
     *
     * @param vendingMachine The RegularVendingMachine associated with this view.
     */
    ...
    /**
     * Updates the item table with data from the Item class.
     */
    ...
    /**
     * Retrieves the item number entered by the user.
     *
     * @return The entered item number as a string.
     */
    ...
    /**
     * Processes a GUI-based transaction based on user input.
     *
    ...
    public boolean processGUIBasedTransaction() {
        try {
            ...
            if (slot < 0 || slot > Item.getItemNames().size()) {
                showMessage("Invalid slot number selected." +  slot);
                return false;
            }
            ...
            while (change == -1) {
                showMessage("Insufficient payment. Please add more to the payment or enter 0 to cancel.");

                String additionalPaymentInput = JOptionPane.showInputDialog(
                        frame,
                        "Enter additional payment denomination:",
                        "Additional Payment",
                        JOptionPane.QUESTION_MESSAGE
                );

                if (additionalPaymentInput == null || additionalPaymentInput.equals("")) {
                    showMessage("Transaction canceled.");
                    return false;
                }

                int additionalPaymentDenomination = Integer.parseInt(additionalPaymentInput);
                if (additionalPaymentDenomination == 0) {
                    showMessage("Transaction canceled.");
                    return false;
                }

                paymentDenomination += additionalPaymentDenomination;
                change = vendingMachine.processTransaction(slot, paymentDenomination);
            }
    ...
    /**
     * Displays a dialog for giving change and updates denomination quantities.
     *
     * @param change            The change amount.
     * @param changeQuantities  The quantities of bills/coins for change.
     * @param denominationQuantities The quantities of bills/coins available.
     * @param denominationValues The values of the bills/coins.
     * @return True if change is given successfully, false otherwise.
     */
    ...
    private class ReceiptDialog extends JDialog {
        ...
        /**
         * Constructs a ReceiptDialog instance with the specified parameters.
         *
         * @param parent                The parent frame.
         * @param slot                  The selected slot number.
         * @param quantity              The quantity of the purchased item.
         * @param change                The change amount.
         * @param denominationQuantities The quantities of bills/coins for change.
         * @param denominationValues    The values of the bills/coins.
         */

...
public class RegularVendingMachine {
    ...
    public RegularVendingMachine() {
        ...
        initializeItems(); // Move this line before setDenominationValues()

        setDenominationValues();
        initializeDenominationQuantities();
        setDenominationNames();
        launchDenominationQuantitiesGUI(parentFrame);
    }
     ...
     */
    public void launchDenominationQuantitiesGUI(JFrame parentFrame) {
        new DenominationQuantitiesGUI(parentFrame, this);
    }
    ...
    private void initializeItems() {
        ...
        List<Integer> itemQuantities = new ArrayList<>(item.getItemQuantities());
    ...
    public void refillItem(int slotNumber, int quantity) {
        ...
        // Retrieve the item based on the slot number
        Item item = items.get(slotNumber - 1);

        // Calculate the remaining capacity of the specific slot
        int remainingCapacity = Item.getITEM_CAPACITY() - item.getItemQuantities().get(slotNumber - 1);
        ...
        int currentQuantity = item.getItemQuantities().get(slotNumber - 1);
        item.getItemQuantities().set(slotNumber - 1, currentQuantity + quantity);
